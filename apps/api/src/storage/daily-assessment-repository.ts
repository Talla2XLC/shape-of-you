...
function evidenceRevisionQuery(personId: string, from: string, to: string) {
  return sql<{ revision: unknown }>`select jsonb_build_object(
    ...
    'workout_activity_links', (select md5(coalesce(string_agg(concat_ws('|', session_id::text, external_activity_id::text, match_basis, match_policy_version), ',' order by session_id), '')) from training_workout_session_activity_links where person_id = ${personId}),

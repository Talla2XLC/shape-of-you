...
BEGIN
  IF EXISTS (
    SELECT 1
      FROM nutrition_brand_versions version
      JOIN nutrition_brands brand ON brand.id = version.brand_id
      ...
      JOIN nutrition_catalog_sources correct_source
        ON correct_source.key = 'fitness_tracker:1yUPcU-2RGIOPyfz8HzR6NSHuztwps81PHbzlGzcK2Ik:2000000006:brand'
     ...
     WHERE brand.visibility = 'private'
       AND wrong_source.key = 'fitness_tracker:1yUPcU-2RGIOPyfz8HzR6NSHuztwps81PHbzlGzcK2Ik:2000000008:brand'
       AND (correct_record.checksum <> wrong_record.checksum
         OR correct_record.parser_version <> wrong_record.parser_version)
  ) THEN
    RAISE EXCEPTION 'Conflicting correct Brand provenance already exists';
  END IF;
END $$;
--> statement-breakpoint
INSERT INTO nutrition_catalog_sources (key, name)
SELECT
  'fitness_tracker:1yUPcU-2RGIOPyfz8HzR6NSHuztwps81PHbzlGzcK2Ik:2000000006:brand',
  'Fitness Tracker Brands'
WHERE EXISTS (
  SELECT 1
    FROM nutrition_brand_versions version
    JOIN nutrition_brands brand ON brand.id = version.brand_id
    JOIN nutrition_catalog_source_records record
      ON record.id = version.source_record_id
    JOIN nutrition_catalog_sources source ON source.id = record.source_id
   WHERE brand.visibility = 'private'
     AND source.key = 'fitness_tracker:1yUPcU-2RGIOPyfz8HzR6NSHuztwps81PHbzlGzcK2Ik:2000000008:brand'
)
...
WHERE brand.id = version.brand_id
  AND brand.visibility = 'private'
  AND wrong_record.id = version.source_record_id
  AND wrong_source.id = wrong_record.source_id
  AND wrong_source.key = 'fitness_tracker:1yUPcU-2RGIOPyfz8HzR6NSHuztwps81PHbzlGzcK2Ik:2000000008:brand'
  AND correct_source.key = 'fitness_tracker:1yUPcU-2RGIOPyfz8HzR6NSHuztwps81PHbzlGzcK2Ik:2000000006:brand'

...
Draft-рекомендация: моделировать authority по типу поля или записи, а не объявлять весь лист Daily_Log авторитетным. Это проще и не переносит смешанные projections как факты. Принятое ADR о cutover с Google Sheets не изменяется.
...
- Какая conflict policy применяется, если два source channels сообщают разные факты за одну дату?

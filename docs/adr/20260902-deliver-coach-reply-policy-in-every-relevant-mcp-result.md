...
- рекомендация может отсутствовать только когда пользователь явно запросил
  сырые данные; если предметный совет нельзя дать безопасно, Coach завершает
  ответ самым безопасным полезным следующим действием по проверенным фактам или
...
- Тесты запрещают условные permission offers, считают acknowledgement-only
  ответ незавершённым и требуют финальный конкретный следующий шаг, сохраняя
  исключение только для явного raw-only запроса.

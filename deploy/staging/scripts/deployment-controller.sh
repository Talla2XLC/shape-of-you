  ...
  case "$input_key" in
    RELEASE_ID|API_DIGEST|IDENTITY_DIGEST|EDGE_DIGEST|CERTBOT_DIGEST|ACME_EMAIL|PUBLIC_IPV4|DEPLOYMENT_TOPOLOGY|SCHEMA_BACKWARD_COMPATIBLE|IDENTITY_SCHEMA_BACKWARD_COMPATIBLE|IDENTITY_OAUTH_CLIENTS_BACKWARD_COMPATIBLE|RUN_WRITE_SMOKE|GHCR_NAMESPACE|GHCR_ACTOR|CONTROL_SHA|DATABASE_URL|IDENTITY_DATABASE_URL|IDENTITY_TOTP_ACTIVE_KEY_ID|IDENTITY_TOTP_ENCRYPTION_KEYS|IDENTITY_OAUTH_ACTIVE_SIGNING_KEY_ID|IDENTITY_OAUTH_SIGNING_KEYS|IDENTITY_OAUTH_COOKIE_KEYS|IDENTITY_CHATGPT_REDIRECT_URI|IDENTITY_WEB_REDIRECT_URI|API_BROWSER_SESSION_KEYS|INTERVALS_ICU_ENABLED|INTERVALS_ICU_CLIENT_ID|INTERVALS_ICU_CLIENT_SECRET|INTERVALS_ICU_REDIRECT_URI|INTEGRATION_ENCRYPTION_KEY_RING|INTEGRATION_ENCRYPTION_ACTIVE_KEY_ID|GHCR_TOKEN)
      ...
      case "$input_key" in
        ...
        INTERVALS_ICU_ENABLED) INTERVALS_ICU_ENABLED=$input_value ;;
...
[ -n "$INTERVALS_ICU_ENABLED" ] || fail 'INTERVALS_ICU_ENABLED is required.'
...
case "$INTERVALS_ICU_ENABLED" in
  true)
    [ -n "$INTERVALS_ICU_CLIENT_ID" ] || fail 'INTERVALS_ICU_CLIENT_ID is required when enabled.'
    [ -n "$INTERVALS_ICU_CLIENT_SECRET" ] || fail 'INTERVALS_ICU_CLIENT_SECRET is required when enabled.'
    [ -n "$INTERVALS_ICU_REDIRECT_URI" ] || fail 'INTERVALS_ICU_REDIRECT_URI is required when enabled.'
    [ -n "$INTEGRATION_ENCRYPTION_KEY_RING" ] || fail 'INTEGRATION_ENCRYPTION_KEY_RING is required when enabled.'
    [ -n "$INTEGRATION_ENCRYPTION_ACTIVE_KEY_ID" ] || fail 'INTEGRATION_ENCRYPTION_ACTIVE_KEY_ID is required when enabled.'
    ...
    ;;
  false) ;;
  *) fail 'INTERVALS_ICU_ENABLED must be true or false.' ;;
esac
...
printf 'INTERVALS_ICU_ENABLED=%s\n' "$INTERVALS_ICU_ENABLED" >> "$runtime_file"
if [ "$INTERVALS_ICU_ENABLED" = true ]; then
  {

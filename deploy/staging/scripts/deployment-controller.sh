...
if [ "$DEPLOY_IDENTITY" = false ]; then
  printf '%s\n' "$EXPECTED_STAGING_BASE" | grep -Eq '^[0-9a-f]{40}$' ||
    fail 'EXPECTED_STAGING_BASE is required when current Identity is reused.'
  ...
  [ "$current_release_id" = "$EXPECTED_STAGING_BASE" ] ||
    fail 'Current release does not match the candidate staging base.'
  ...
  [ -z "$EXPECTED_STAGING_BASE" ] ||
    fail 'EXPECTED_STAGING_BASE must be omitted when Identity deployment is enabled.'
